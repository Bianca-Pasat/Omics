...
rownames(te)<-te$ensembl_gene_id
counts=te[,(7:9)]
samples_vector_file=c(rep("MKC_24",3))
y <- DGEList(counts=counts, samples=samples_vector_file)
...
keep.exprs <- aveLogCPM(y) > 4
y <- y[keep.exprs, keep.lib.sizes=FALSE]
write.table(y$counts,"/home/bianca/Desktop/mRNA/marray_24_mkc_filtered.txt",quote=F, row.names = T,sep="\t")
