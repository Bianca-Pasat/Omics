...
mem<-read.table("/home/bianca/Desktop/membrane_residing_DE_genes_RNASEQ_MKC_VS_DMSO/IDS/all_ids.txt", sep="\t")
...
### proteomics up regulated all time points
seq<-getSequence(id=c("DNAH1", "HSP90AB4P", "AGPAT5", "CCDC23","ARL6IP4"), type="hgnc_symbol", seqType="cdna",mart=homo.anno)
seq<-getSequence(id="CDC23", type="hgnc_symbol", seqType="cdna",mart=homo.anno)

exportFASTA(seq, file="/home/bianca/Desktop/cdcup_across_tp_prot.fasta")
...
genes_mem <- getBM(attributes = attributes, filters=filters,values = values, mart = homo.anno)

write.table(genes_mem$ensembl_gene_id,"/home/bianca/Desktop/membrane_residing_DE_genes_RNASEQ_MKC_VS_DMSO/IDS/all_ids_ens.txt",sep="\t",quote=F,row.names = F)
write.table(genes_mem,"/home/bianca/Desktop/membrane_residing_DE_genes_RNASEQ_MKC_VS_DMSO/IDS/all_ids_ens_hgnctxt",sep="\t",quote=F,row.names = F)
