...
#######################################################
pd=propd(X1, X_info$joint, alpha=NA, p =100)
pd=propd(X1, X_info$joint, alpha=0.01,weighted = TRUE, p =100)

theta_d=setDisjointed(pd)
theta_e=setEmergent(pd)

theta_d=updateCutoffs(theta_d,cutoff = seq(.05,.95,.3), ncores=7)
theta_e=updateCutoffs(theta_e,cutoff = seq(.05,.95,.3), ncores=7) ### CHECK THE VINGETTE TO SEE IF FDR MAKES SENSE


theta_d=updateCutoffs(theta_d,cutoff = seq(.001,.005,.001), ncores=7)
theta_e=updateCutoffs(theta_e,cutoff = seq(.001,.005,.001), ncores=7) 

theta_e=setActive(theta_e, what="theta_f")
pd=updateF(theta_e, moderated = TRUE, ivar="clr")
pd=updateF(theta_e, moderated = TRUE, ivar="iqlr")
pd=updateF(theta_e, moderated = TRUE, weigthened=TRUE, ivar="alr")
pd=updateF(theta_e, moderated = TRUE, ivar="clr")
pd@results$Fstat

pd.nn=updateF(theta_d, moderated = TRUE, ivar="clr")
pd.nn@results$Fstat
length(pd.nn@results$Fstat[which(pd.nn@results$Fstat > 4.5)])


###

pd=updateF(pd, moderated = TRUE, ivar="iqlr")
pd.nn=updateF(pd, moderated = TRUE, ivar="alr")
...
## four variants state of theta
pd.nn=propd(X, X_info$condition, weighted=FALSE, p =100)
pd.wn=propd(X, X_info$condition, weighted=TRUE, alpha=NA, p =100)
pd.na=propd(X, X_info$condition, weighted=FALSE, alpha=.01, p =100)
pd.wa=propd(X, X_info$condition, weighted=TRUE, alpha=.01, p =100)


## moderating F statistics
pd.nn=updateF(pd.nn, moderated = FALSE, ivar="clr")
pd.nn@results$Fstat
pd.wn=updateF(pd.wn, moderated = FALSE, ivar="clr")
pd.wn@results$Fstat
pd.na=updateF(pd.na, moderated = FALSE, ivar="clr")
pd.na@results$Fstat
pd.wa=updateF(pd.wa, moderated = FALSE, ivar="clr")
pd.wa@results$Fstat
...
##### a brief critique on proportionality
Y= X /rowSums(X) * abs(rnorm((nrow(X))))

# spurious correlation
pairs(X) # absolute data
pairs(Y) # relatice data

## coefficients
cor(X)
cor(Y)

## in depth look at VLR
propr:::proprVLR(Y[,1:4])
propr:::proprVLR(X)

## in depth look at CLR
propr:::proprCLR(Y[,1:4])
propr:::proprCLR(X)

## in depth look at ALR
pairs(propr:::proprALR(Y, ivar=5)) # relative alr
pairs(X) # absolute data
propr(Y, ivar=5)@matrix # relative propotionality with alr