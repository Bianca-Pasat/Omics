...
genesign<-read.table("/Users/user/NUIG-HCT116/WPT/Gene_Prioritization.tsv",  header = T,  sep="\t")[,2]
genesign.FC<-read.table("/Users/user/Downloads/HCT_116_MRNA-MIRNA/mrna-mirna/results_combined_for_BIO/wpt.txt",  header = F, row.names=1, sep="\t")[,c(1,2)]
...
TCGAanalyze_survival(data = colData(exp),
                     ...
                     filename = "/Users/user/wpt_survival.jpg",#or "survivalPlot.png"
                     #height = 10,
                     #width = 10,
                     dpi = 1200,
...
colnames(testx) <- colnames(logCPM.genesign)
