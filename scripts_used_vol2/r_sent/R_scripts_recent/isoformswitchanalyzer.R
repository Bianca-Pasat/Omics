...
SwitchListDexseq <- isoformSwitchTestDEXSeq( SwitchList ) 
SwitchListDexseq<-extractSequence(SwitchListDexseq)
SwitchListDexseq<- isoformSwitchAnalysisPart2(
  switchAnalyzeRlist        = SwitchListDexseq,
  n                         = 10,
  removeNoncodinORFs        = TRUE,
  pathToCPATresultFile      =  "/home/bianca/Downloads/part1/cpat.txt",
  pathToPFAMresultFile      =  "/home/bianca/Downloads/part1/pfam.txt",
...
extractTopSwitches(SwitchListDexseq, filterForConsequences = TRUE, n=Inf)
...
load("/home/bianca/Downloads/part1/isoforms_last_with_dexseq_boxpplots.RData")
...
wowla1<-function(unique_gene_names,SwitchList,c1,c2){
  for (i in seq(1,length(unique_gene_names),1)){
    switchPlot(SwitchList, gene = unique_gene_names[i],condition1 = c1,condition2 = c2,dIFcutoff = 0)
  }
}

keep_only<-function(condition_1.x,recond,gene_log2_fold_change.x,num,hey){
  heyou<-hey[hey$condition_1.x ==recond, ]
  heyou<-heyou[heyou$gene_log2_fold_change.x > num,]
  lala<-unique(heyou$gene_name)
  return(lala)
}

keep_neg<-function(condition_1.x,recond,gene_log2_fold_change.x,num,hey){
  heyou<-hey[hey$condition_1.x ==recond, ]
  heyou<-heyou[heyou$gene_log2_fold_change.x < num,]
  lala<-unique(heyou$gene_name)
  return(lala)
...
keep_common_up<-function(condition_1.x,recond,gene_log2_fold_change.x,num,hey){
  heyou<-hey[hey$condition_1.x ==recond, ]
  heyou<-heyou[heyou$gene_log2_fold_change.x < num,]
  lala<-unique(heyou$gene_name)
  return(lala)
...
wow<-is_it(SwitchList, confirmed_exons_cor)
hey<-up_down_seq(wow)

### runs DEXSEQ
wowd<-is_it(SwitchListDexseq, exact)
wowd<-is_it(SwitchListDexseq, confirmed_exons_cor)
heyd<-up_down_seq(wowd)

# check that no transcripts exist in the first fasta file
idsisoforms<-read.table("/home/bianca/Downloads/part1/header_isforms")
#### CHECK IF SOME OF ISOFORM.Y IS NOT IN THE ISOFORMS HEADER FILE
length(which(unique(hey$isoform_id.y) %in% idsisoforms$V1))==length(unique(hey$isoform_id.y))

# DRIMseq
ugn<-unique(hey$gene_name)
ugn<-unique.data.frame(hey)

# Dexseqseq
ugnd<-unique(heyd$gene_name)
ugnd<-unique.data.frame(heyd)

common<-ugn[which(ugn %in% ugnd)]
common<-ugn[which(ugn$gene_name %in% ugnd$gene_name),]
###
common<-hey[which(hey$gene_name %in% heyd$gene_name),]
nonacommon<-na.omit(common$gene_name)

nocommon1<-ugn[-(which(ugn %in% ugnd))]
nocommon2<-ugnd[-(which(ugnd %in% ugn))]



write.table(ugn,"/home/bianca/Downloads/part1/important_transcripts_for_bio.txt", sep="\t", quote=FALSE, row.names = FALSE)
write.table(ugnd,"/home/bianca/Downloads/part1/important_transcripts_for_bio_dexseq.txt", sep="\t", quote=FALSE, row.names = FALSE)
write.table(common,"/home/bianca/Downloads/part1/important_transcripts_for_bio_common_elaborate.txt", sep="\t", quote=FALSE, row.names = FALSE)

### matthieu

write.table(hey[,c(1,4,6,7,27,28,38,41,64,65,75)],"/home/bianca/Downloads/part1/Matthieu_important_transcripts_for_bio.txt", sep="\t", quote=FALSE, row.names = FALSE)
write.table(heyd[,c(1,4,6,7,27,28,38,41,64,65,75)],"/home/bianca/Downloads/part1/Matthieu_important_transcripts_for_bio_dexseq.txt", sep="\t", quote=FALSE, row.names = FALSE)
write.table(common[,c(1,4,6,7,27,28,38,41,64,65,75)],"/home/bianca/Downloads/part1/Matthieu_important_transcripts_for_bio_common_elaborate.txt", sep="\t", quote=FALSE, row.names = FALSE)


wowla1(ugn,wow,"D8","M8")
wowla1(ugn[2],wow,"D8","M8")
wowla1(ugn[2],wow,"D24","M24")

wowla1(ugn[8],wow,"D8","M8")
wowla1(ugn[8],wow,"D24","M24")

wowla1(common,wow,"D8","M8")
wowla1(common,wow,"D24","M24")

wowla1(nocommon1,wow,"D8","M8")
wowla1(nocommon1,wow,"D24","M24")

wowla1(nocommon2[1],wow,"D8","M8")
wowla1(nocommon2[1],wow,"D24","M24")


upgenes8<-keep_only(condition_1.x,"D8",gene_log2_fold_change.x ,0,hey)
upgenes24<-keep_only(condition_1.x,"D24",gene_log2_fold_change.x ,0,hey)

downgenes8<-keep_neg(condition_1.x,"D8",gene_log2_fold_change.x ,0,hey)
downgenes24<-keep_neg(condition_1.x,"D24",gene_log2_fold_change.x ,0,hey)

wowla1(upgenes8[6],wow,"D8","M8")
wowla1(upgenes24[6],wow,"D24","M24")

wowla1(downgenes8,wow,"D8","M8")
wowla1(downgenes24,wow,"D24","M24")



# MERGE
ahha<-merge(SwitchList$isoformFeatures ,confirmed_exons_cor, by.x = 3, by.y = 1)
